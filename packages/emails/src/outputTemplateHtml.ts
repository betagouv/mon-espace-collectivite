...
export async function outputHtmlTemplate(name: string, htmlContent: string): Promise<void> {
  await outputDirectoryIfItDoesNotExist();
  await fs.writeFile(`${outputDirectory}/${name}.html`, htmlContent);
}

export async function outputMjmlTemplate(name: string, mjmlTemplate: string): Promise<void> {
...
export function createHtmlTemplateOutput(templateName: string): (htmlContent: string) => Promise<void> {
  let index = 0;
 ...
 */
export function createMjmlTemplateOutput(templateName: string): (templateName: string) => Promise<void> {
  let index = 0;
