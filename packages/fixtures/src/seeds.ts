...
const deleteAll = async (transaction: TransactionClient) => {
  const tables = await transaction.$queryRaw<{ table_name: string }[]>`SELECT table_name
  ...
  await transaction.$queryRawUnsafe(`TRUNCATE TABLE "${tables.map(({ table_name }) => table_name).join('", "')}" CASCADE`);
...
const main = async (eraseAllData: boolean, random?: number) => {
  await prismaClient.$transaction(async (transaction) => {
    ...
    console.log(`Generating ${random ? `${random} set of random` : 'non-random fixtures'} data`);
...
const program = new Command()
  ...
  .option('-r, --random [number]', 'Number of random items to seed', myParseInt, 0);
