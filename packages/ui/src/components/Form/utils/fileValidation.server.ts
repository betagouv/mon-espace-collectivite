...
export const fileValidation = ({
  ...
  const sizeValidation = maxSizeInBytes ? fileSizeValidation(maxSizeInBytes, maxSizeMessage) : undefined;

  const typeValidation = acceptedMimeTypes ? fileMimeTypeValidation(acceptedMimeTypes, acceptedMimeTypesMessage) : undefined;
  ...
  return z.object(
    {
      ...
      mimeType:
        acceptedMimeTypes && typeValidation
          ? z.string().refine((value) => !value || typeValidation.safeParse(value).success, acceptedMimeTypesMessage)
          : z.string(),
      name: z.string(),
      size:
...
export const optionalFileValidation = (options: ServerFileValidationOptions) => fileValidation(options).nullable();
