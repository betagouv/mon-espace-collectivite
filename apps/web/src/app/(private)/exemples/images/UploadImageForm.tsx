...
import { FileValidationOptions, fileValidation } from '@app/ui/components/Form/utils/fileValidation.client';
...
type UploadFileFormData = z.infer<ReturnType<typeof createUploadFileFormValidation>>;

const UploadImageForm = ({ ...fileValidationOptions }: FileValidationOptions) => {
  const validation = useMemo(() => createUploadFileFormValidation(fileValidationOptions), [fileValidationOptions]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <FileFormField control={control} path="file" disabled={disabled} info={uploadProgressInfo} />
      {fileUpload.error && <p className={classNames('fr-error-text', 'fr-mt-1v')}>{fileUpload.error}</p>}
      {isSuccessful && <p className={classNames('fr-valid-text', 'fr-mt-1v')}>Image envoyée</p>}
