...
import { UserSignup, UserSignupValidation } from '@app/web/server/rpc/user/userSignup';
...
const EmailSignupForm = ({ error, callbackUrl, email }: { error?: string; email?: string; callbackUrl: Route }) => {
  ...
  return (
    <form id="signup-with-email" onSubmit={form.handleSubmit(onSubmit)}>
      ...
      <InputFormField control={form.control} path="firstName" label="Prénom" disabled={disabled} />
      <InputFormField control={form.control} path="lastName" label="Nom" disabled={disabled} />
      <ButtonsGroup
        className="fr-mt-12v"
        buttons={[
          {
